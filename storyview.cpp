...
void StoryView::show(Renderer& renderer)
{
  ...
  const std::string s = Time(nullptr).asString() + std::string(" Player has won: \'s\'. Bubbles: %d, Stars: %d");
  ...
  try {
    Model::QueryResult *qres = mod->story.all();
    int k = 0;
    ...
    renderer.drawNumber(statsp + m_stats.markers().at(1), (int)linfo->bubleWins());
    renderer.draw(statsp + m_stats.markers().at(3), "%d%c", int(double(linfo->bubleWins()) / (double)qres->size() * 100), '%');

    renderer.drawNumber(statsp + m_stats.markers().at(2), (int)linfo->starWins());
    renderer.draw(statsp + m_stats.markers().at(4), "%d%c", int(double(linfo->starWins()) / double(qres->size()) * 100), '%');
